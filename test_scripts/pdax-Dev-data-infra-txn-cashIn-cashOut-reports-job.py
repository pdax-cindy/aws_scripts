...
p_date
FROM(
SELECT substr(input_file_name,48,10) as p_date,
email as username,
...
row_number()OVER (partition BY email,reference_number ORDER BY update_date DESC) AS rown
    FROM payments
    WHERE status = 'success'
    AND email NOT LIKE '%osl.com'
    AND email NOT LIKE '%bc.holdings'
    )
WHERE rown = 1
AND (
...
credit_ccy,
cast(credit_amount as decimal(19,4)) as credit_amount,
debit_ccy,
cast(debit_amount as decimal(19,4)) as debit_amount,
update_date,
from_utc_timestamp(update_date, 'Asia/Manila') as transactiondatetime,
substr(input_file_name,69,10) as input_file_name_date,
filename_DATE as otc_filename,
substr(input_file_name,69,10) as p_date,
date_sub(current_date,"""+dateDelay+""") as date_delay,
"""+jobRunMode+""" as jobRunMode,
from_utc_timestamp('{}', 'Asia/Manila') as process_date_timestamp
from 
...
WHERE
(CASE WHEN jobRunMode = 1 then ( (p_date < date_sub(date_delay,1)) AND (input_file_name_date = otc_filename) )
...
#final_df.repartition(1).write.partitionBy("p_date").mode("overwrite").option("header","true").csv("s3://pdax-data-dev-team-bucket-ap-southeast-1/master-transaction-report/cashin_out/")
